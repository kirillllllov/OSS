...
export async function generateBallotBuffer(
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // 4. ТАБЛИЦА ВОПРОСОВ
  //    Структура: [QR(15) | текст(107) | ЗА(18) | ПРОТИВ(18) | ВОЗДЕРЖАЛСЯ(22)]
  //    Ячейки голосования — горизонтально, фиксированного размера
  // ═══════════════════════════════════════════════════════════════════════════

  // Вспомогательная функция: ячейка-метка голосования в шапке
  const voteHeaderCell = (label: string, colIdx: number) =>
    tc(
      [
        para(
  ...
  // Ширины трёх vote-колонок и их суммарная ширина
  const VOTE_WIDTHS = [Q_COL[2], Q_COL[3], Q_COL[4]] as const; // [18, 18, 22] мм
  const VOTE_TOTAL = VOTE_WIDTHS[0] + VOTE_WIDTHS[1] + VOTE_WIDTHS[2]; // 58 мм

  // Объединённая ячейка голосования (columnSpan=3) с вложенной таблицей из 3 боксов
  const makeVoteMergedCell = (): TableCell => {
    const innerRow = new TableRow({
      height: { value: mm(16), rule: 'exact' },
      children: VOTE_WIDTHS.map(
        (w) =>
          new TableCell({
            children: [emptyPara()],
            width: { size: mm(w), type: WidthType.DXA },
            borders: BORDERS_ALL as any,
    ...
    const innerTable = new Table({
      width: { size: mm(VOTE_TOTAL), type: WidthType.DXA },
      columnWidths: [...VOTE_WIDTHS].map(mm),
  ...
  // ── Строка-шапка: QR | «Содержание вопроса» | ЗА | ПРОТИВ | ВОЗДЕРЖАЛСЯ ──
  ...
  questionRows.push(
    new TableRow({
      ...
      children: [
        ...
        voteHeaderCell('ЗА', 2),
        voteHeaderCell('ПРОТИВ', 3),
        voteHeaderCell('ВОЗДЕРЖ.\u200BАЛСЯ', 4),
      ],
    }),
  );
  ...
  for (let i = 0; i < sm.agendaItems.length; i++) {
    ...
    // QR-ячейка вопроса — выравнивание по верхнему краю по центру
    ...
    const qrCell = new TableCell({
      ...
      borders: BORDERS_ALL,
      verticalAlign: VerticalAlign.TOP,
      margins: { top: 60, bottom: 10, left: 10, right: 10 },
    });

    // Ячейка текста вопроса
    const textCell = tc(
      [para([tr(qText, { size: FONT_SIZES.sm })], AlignmentType.LEFT)],
      {
        widthMm: Q_COL[1],
        borders: BORDERS_ALL,
    ...
    questionRows.push(
      new TableRow({
        // Минимальная высота строки вопроса — чтобы боксы голосования были заметны
        height: { value: mm(20), rule: 'atLeast' },
        children: [qrCell, textCell, makeVoteMergedCell()],
      }),
    );
  }
